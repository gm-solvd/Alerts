...
class ScanService(
    ...
    fun breachScan(userId: UUID, profile: UserScanProfile): List<Alert> {
        ...
        val alerts = allBreaches.map { breach ->
            alertRepository.save(
                Alert(
                    ...
                    description = "Your data was found in the ${breach.name} breach (${breach.breachDate}). " +
    ...
    fun identityScan(userId: UUID, profile: UserScanProfile): List<Alert> {
        val results = identityExposureScanner.scan(profile.email, profile.fullName)

        val alerts = results.map { result ->
            alertRepository.save(
                Alert(
                    userId = userId,
                    ...
                    description = "Your profile was found on ${result.source} (${result.profileUrl}). " +
    ...
    fun piiExposureScan(userId: UUID, profile: UserScanProfile): List<Alert> {
        ...
        val alerts = results.map { result ->
            ...
            alertRepository.save(
                Alert(
                    ...
                    description = "Your personal information was found on ${result.source} (${result.sourceUrl}). " +
    ...
    fun socialFootprintScan(userId: UUID, profile: UserScanProfile, username: String?): List<Alert> {
        ...
        val alerts = results.map { result ->
            alertRepository.save(
                Alert(
                    ...
                    description = "Your profile '${result.username}' was found on ${result.platform} (${result.profileUrl}). " +
    ...
    fun fullScan(userId: UUID, profile: UserScanProfile, username: String?): List<Alert> {
