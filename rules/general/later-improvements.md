...
- [ ] **[ARCH]** `DashboardViewModel`, `AlertsViewModel` — `AppError.Unauthorized` sets `Error(message)` state but does not trigger navigation to login; add a `SessionExpired` UiState variant so screens can auto-navigate on token expiry (from PR #36 review)
