...
- [x] **[STYLE]** `AlertsScreen.kt` + `DashboardScreen.kt` — `CredentialExposedBadge()` and `DashboardCredentialBadge()` are identical composables; extract to shared `ui/component/CredentialExposedBadge` (from PR #55 review)
- [x] **[STYLE]** `iosApp/PrivacyAlert/Info.plist` — `UILaunchStoryboardName` references `LaunchScreen` but no storyboard file exists; add a `LaunchScreen.storyboard` with centered shield logo for proper iOS launch experience (from PR #61 review)
